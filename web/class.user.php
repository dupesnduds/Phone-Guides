	...
	class User extends Database
	{
      ...
      function __construct() {
		...
		***/
		...
		{			
         ...
         $sql=sprintf("INSERT INTO users (username, password, role_id) VALUES ('%s', '%s', '%s')", $mysql['username'], $mysql['password'], $mysql['role_id']);
         $this->_db->query($sql);
         $this->_db->close();
		}
      ...
      ***/
      ...
      {        
         ...
         $clean['username'] = $this->_sanitize->stripTagsEncode($username);
         $mysql['username'] = $this->_db->real_escape_string($clean['username']);
         $sql = sprintf("SELECT id FROM users WHERE username = '%s' LIMIT 1", $mysql['username']);
      ...
      ***/
      ...
      {        
         ...
         $mysql['username'] = $this->_db->real_escape_string($clean['username']);
			...
			$sql = sprintf("SELECT id FROM users WHERE username = '%s' AND id = %s LIMIT 1", $mysql['username'], $mysql['id']);
      ...
      ***/
      ...
      {        
         ...
         $sql=sprintf("UPDATE users SET username = '%s', role_id = '%s' WHERE id = '%s'", $mysql['username'], $mysql['role_id'], $mysql['id']);
         $this->_db->query($sql);
         $this->_db->close();
      }
      ...
      ***/
      ...
      {        
         ...
         $sql=sprintf("UPDATE users SET password = '%s' WHERE id = '%s'", $mysql['password'], $mysql['id']);
         $this->_db->query($sql);
         $this->_db->close();
      }
      ...
      ***/
      ...
      {        
         ...
         $sql=sprintf("UPDATE users SET role_id = '%s' WHERE id = '%s'", $mysql['role_id'], $mysql['id']);
         $this->_db->query($sql);
         $this->_db->close();
      }
      ...
      ***/
      ...
      {        
         ...
         $sql=sprintf("DELETE FROM users WHERE id = '%s'", $mysql['id']);
         $this->_db->query($sql);
         $this->_db->close();
      }
      ...
      ***/
      ...
      {     
         ...
         $sql = "SELECT id, role FROM roles ORDER BY id";
         ...
         if($query = $this->_db->query($sql))
         {
            $i = 1;
               ...
               //a bit of formatting to ensure commas are in their correct places
					...
					$sql2 = sprintf("SELECT id, username FROM users WHERE role_id = %s ORDER BY username", $this->_sanitize->stripTagsEncode($row['id']));
